...
async function initializeAppState(currentView ,
    ...
    groupIdParam = URLSearchParams.get("groupid");
...
function renderTabs() {
    ...
    minhasListasTab.textContent = 'Meus Listas';
...
function renderListaListas() {
    ...
    titulo.textContent = 'Meus Listas';
    ...
    if (appState.listas.length === 0 || appState.mostrarListasVazios) {
        ...
        appState.listas.forEach(grupo => {
            ...
            groupItem.addEventListener('click', (e) => {
                e.stopPropagation();
                window.location.href = `products.html?groupid=${groupIdParam}&listid=${grupo.group_id}`;
            })
...
async function startApp(currentView = "listaListas",
    ...
    await initializeAppState(currentView ,
        activeTab,
        listId,
        mostrarListasVazios); // Espera carregar dados
